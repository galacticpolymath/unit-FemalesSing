...
#Parameter descriptions:
  ...
  # dest_folder: needs to be like "figures/spectrograms/" to refer within working directory; goes to soundFile folder by default
...
testSpec<-function(soundFile,dest_folder,outFilename,colPal="inferno",crop=NULL,bg=NULL,filter=NULL,xLim=NULL,yLim=c(0,10),plotLegend=F,onlyPlotSpec=T,ampTrans=1,min_dB=-30,wl=512,ovlp=90,wn="blackman",specWidth=6,specHeight=2,colbins=30,ampThresh=0,bgFlood=F,fontAndAxisCol=NULL,optim=NULL,...)
  {
...
########
rspectVid <-function(specParams,vidName,framerate=30,highlightCol="#4B0C6BFF",highlightAlpha=.6,cursorCol="#4B0C6BFF",delTemps=T,... )
{
...
if(!missing(vidName)){
    ...
    vidName=paste0(specParams$dest_folder,iName0,".mp4")
    }else{
  ...
  tempdir<-paste0(specParams$dest_folder,"temp/")
...
#s<-'/Users/mattwilkins/GDrive/My Research/My Papers/Female Song in BARS/Data/song exemplars/F2-2_2540-98915_#26 Fisher Dwidle Dwight-01_(MRW01156_Master metafile).wav'



# onlyPlotSpec = F;bgFlood=T;ampTrans=3;fontAndAxisCol = "cyan";plotLegend = T;colPal=c("black","cyan");crop=11;xLim=3;yLim=c(0,.7)
s="http://www.oceanmammalinst.org/songs/hmpback3.wav"

# t<-testSpec(s,onlyPlotSpec = F,bgFlood=T,ampTrans=3,fontAndAxisCol = "cyan",plotLegend = T,colPal=c("black","cyan"),xLim=3,yLim=c(0,.7),crop=11)  
# specParams=t
# rspectVid(t,delTemps = F)


#batch a bunch of files
f<-list.files("/Users/mattwilkins/GDrive/GP/Units/unit-FemalesSing/Lessons/Science/OZtourAssets/Songs, both sexes/",pattern=".wav|.mp3",full.names=T) 

# look for file names
sapply(f,function(x) substr(basename(x),1,6),USE.NAMES = F)

# lapply(f[-1],function(x){
#   t<-testSpec(x,crop=F,xLim=3,onlyPlotSpec = F,bgFlood=T,ampTrans=1.5,min_dB=-40)  
#   rspectVid(t,delTemps=F)
# })

#BAOR F
# x=f[1]
# testSpec(x,xLim=3,crop=15,onlyPlotSpec = F,bgFlood=T,ampTrans=2,min_dB=-40) %>% rspectVid(delTemps=T)

#BAOR M
# x=f[2]
# tmp<-testSpec(x,xLim=3,crop=15,onlyPlotSpec = F,bgFlood=T,ampTrans=2,min_dB=-40)  
# rspectVid(tmp,delTemps=T)

# #BARS F
#  x=f[3]
#  testSpec(x,xLim=3,crop=c(.5,8),onlyPlotSpec = F,bgFlood=T,ampTrans=2,min_dB=-40) %>% rspectVid(delTemps=T)

# #BARS M
#  x=f[4]
# testSpec(x,xLim=3,filter=c(0,1.), crop=F,onlyPlotSpec = F,bgFlood=T,ampTrans=2.8,min_dB=-40) %>% rspectVid(delTemps=F)
...
# #BUOR M
# x=f[8]
# t<-testSpec(x,xLim=3,crop=15,onlyPlotSpec = F,bgFlood=T,ampTrans=2,min_dB=-40) 
# rspectVid(t,delTemps=F)
# 
# #CARW F
# x=f[9]
...
#CNWR Duet
# x=f[12]
# testSpec(x,xLim=3,crop=18,onlyPlotSpec = F,bgFlood=T,ampTrans=2,min_dB=-40) %>% 
#   rspectVid(delTemps=T,highlightCol=viridis(10,option="inferno")[3],cursorCol=viridis(10,option="inferno")[3])
  
#CNWR F
# x=f[13]
# testSpec(x,xLim=3,crop=c(1,19),onlyPlotSpec = F,bgFlood=T,ampTrans=2,min_dB=-40) %>% rspectVid(delTemps=T)
...
# #COYE F
# x=f[15]
# testSpec(x,xLim=3,crop=3,onlyPlotSpec = F,bgFlood=T,ampTrans=2,min_dB=-40) %>% rspectVid(delTemps=T)

# #COYE M
# x=f[16]
# testSpec(x,xLim=3,crop=c(.5,12),onlyPlotSpec = F,bgFlood=T,ampTrans=2,min_dB=-40) %>% rspectVid(delTemps=T)

# #HOFI F
# x=f[17]
# testSpec(x,xLim=3,crop=12,onlyPlotSpec = F,bgFlood=T,ampTrans=2,min_dB=-40) %>% rspectVid(delTemps=T)
...
# #HOWR F
# x=f[19]
# testSpec(x,xLim=3,crop=9,onlyPlotSpec = F,bgFlood=T,ampTrans=2,min_dB=-40) %>% rspectVid(delTemps=T)

# #HOWR M
# x=f[20]
# testSpec(x,xLim=3,crop=c(1,14),onlyPlotSpec = F,bgFlood=T,ampTrans=2,min_dB=-40) %>% rspectVid(delTemps=T)
...
#NOCA M
# x=f[22]
# testSpec(x,xLim=3,crop=c(1.2,16.2),onlyPlotSpec = F,bgFlood=T,ampTrans=2,min_dB=-40) %>% rspectVid(delTemps=T)

# #PRWA F
# x=f[23]
# testSpec(x,xLim=3,crop=9,onlyPlotSpec = F,bgFlood=T,ampTrans=2,min_dB=-40) %>% rspectVid(delTemps=T)

# #PRWA M #keeps crashing Studio!
# x=f[24]
# testSpec(x,xLim=3,crop=F,onlyPlotSpec = F,bgFlood=T,ampTrans=2,min_dB=-40) %>% rspectVid(delTemps=T)

# #RUBL F
# x=f[25]
# testSpec(x,xLim=3,crop=15,onlyPlotSpec = F,bgFlood=T,ampTrans=2,min_dB=-40) %>% rspectVid(delTemps=T)#RUBL M
...
#SCTA M
# x=f[30]
# testSpec(x,xLim=3,filter=c(0,.8),crop=12,onlyPlotSpec = F,bgFlood=T,ampTrans=2,min_dB=-40) %>% rspectVid(delTemps=T)
...
# 
# plot(1:10,1:10,cex=5,col=viridis(10,option="inferno"),pch=19,xlim=c(0,11),ylim=c(0,11))
# rect<-data.frame(xmin=2.6,xmax=3,ymin=0,ymax=10,alpha=.9)
# t$spec[[1]]+geom_rect(data=rect,aes(xmin=xmin,ymin=ymin,xmax=xmax,ymax=ymax),alpha=.9,fill=viridis(10,option="inferno")[2],inherit.aes=F)+geom_segment(x=2.6,xend=2.6,y=0,yend=10,col=viridis(10,option="inferno")[3],inherit.aes=F)
